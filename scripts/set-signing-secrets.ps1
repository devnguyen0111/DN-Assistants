...
try {
  ...
  Get-Content -Raw $keyPath | & $gh.Source secret set TAURI_SIGNING_PRIVATE_KEY --repo devnguyen0111/DN-Assistants
  if ($password) {
    $password | & $gh.Source secret set TAURI_SIGNING_PRIVATE_KEY_PASSWORD --repo devnguyen0111/DN-Assistants
  } else {
